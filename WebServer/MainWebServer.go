...
func ShowServerInfo(){ //输出当前网站配置信息
...
func WebRequestReceiveMonitor(ch chan int,ServerNumber int){ //Web请求监控进程
	i := 0
...
func PathExists(path string) (bool) { //文件是否存在
	_, err := os.Stat(path)
	if err == nil {
...
func HTTP404(WebRoot string) (string,[]byte) { //404错误返回
...
func WebRequestReceive(r *ghttp.Request) {
	ch := make(chan int) //设置信号量
	r.ParseForm()
	var DataKey string
	go WebRequestReceiveMonitor(ch,0) //启动监控进程
	...
	Flag := 0 // 0为正常状态 1为缺少访问文件已添加默认文件状态 -1为404状态
	Path := r.URL.Path
	...
	if Flag >= 0{
		if PathExists(WebRoot+Path){
			if !isAsset(Path,WebNumber){
				...
				if FileType == ".SEHP"{
					// 返回ContentType(类型string),EchoData(类型[]byte)
					hash := md5.New()
					...
					NowData.ID = DataKey
					NowData.r = r
					AllEnterData[DataKey] = NowData//暂存数据
...
func compressStr(str string) string { //删首尾空
    if str == "" {
        return ""
    }
...
func CommunicationEngine(){
	...
	for {
		...
		/*if strings.Compare(RecvData[1],"test")==0{
			_, err = listener.WriteToUDP([]byte(RecvData[0]+"|DEBUG:"+RecvData[0]+"-"+RecvData[1]), remoteAddr)
			if err != nil {
			...
			/*else if strings.Compare(RecvData[0],"EncryptAES") == 0{
				key:=[]byte("sehpisbest")
				_, err = listener.WriteToUDP([]byte("EncryptAES|"+string(encryptAES([]byte(RecvData[1]),key)[:])), remoteAddr)
				if err != nil {
					fmt.Printf(err.Error())
				}
			} else if strings.Compare(RecvData[0],"DecryptAES") == 0{
				key:=[]byte("sehpisbest")
				_, err = listener.WriteToUDP([]byte("EncryptAES|"+string(decryptAES([]byte(RecvData[1]),key)[:])), remoteAddr)
				if err != nil {
					fmt.Printf(err.Error())
				}
			}*/
			/*else if strings.Compare(RecvData[0],"GetFile") == 0{
				if _, ok := AllEnterData[RecvData[1]];ok{
					Temp := AllEnterData[RecvData[1]]
					if f, h, e := Temp.r.FormFile(RecvData[2]); e == nil {
						defer f.Close()
						name := gfile.Basename(h.Filename)
						_, err = listener.WriteToUDP([]byte("GetFile|"+name), remoteAddr)
						if err != nil {
							fmt.Printf(err.Error())
						}
					} else {
						_, err = listener.WriteToUDP([]byte("GetFile|FileNULL"), remoteAddr)
						if err != nil {
							fmt.Printf(err.Error())
						}
					}
				} else {
					_, err = listener.WriteToUDP([]byte("GetFile|IDNULL"), remoteAddr)
					if err != nil {
						fmt.Printf(err.Error())
					}
				}
			} else if strings.Compare(RecvData[0],"SaveFile") == 0{
				if _, ok := AllEnterData[RecvData[1]];ok{
					Temp := AllEnterData[RecvData[1]]
					if f, h, e := Temp.r.FormFile(RecvData[2]); e == nil {
						defer f.Close()
						name   := gfile.Basename(h.Filename)
						_, err = listener.WriteToUDP([]byte("SaveFile|Success"), remoteAddr)
						fmt.Println(RecvData[4]+"\\" + name);
						if err != nil {
							fmt.Printf(err.Error())
						}
						buffer := make([]byte, h.Size)
						f.Read(buffer)
						gfile.PutBinContents(RecvData[4]+"\\" + name, buffer)
					} else {
						_, err = listener.WriteToUDP([]byte("SaveFile|FileNULL"), remoteAddr)
						if err != nil {
							fmt.Printf(err.Error())
						}
					}
				} else {
					_, err = listener.WriteToUDP([]byte("SaveFile|IDNULL"), remoteAddr)
					if err != nil {
						fmt.Printf(err.Error())
					}
				}
			} else if strings.Compare(RecvData[0],"GetFileInfo") == 0{
...
func execSEHP(WebNumber int,DataKey string,Path string,r *ghttp.Request) []byte{
	...
	for i:=0;i<len(CodeOriginal);i++{
		if strings.HasPrefix(CodeOriginal[i],"#!"){ //检测预处理设置
		...
		} else if strings.HasPrefix(CodeOriginal[i],"#"){ //检测注释
			//NULL
		} else {
			if strings.Contains(CodeOriginal[i], "<#sehp"){ //发现SEHP预处理标记
				...
				if strings.Contains(CodeOriginal[i], "src="){
					...
					for !strings.Contains(CodeOriginal[i], "#>"){ //寻找SEHP预处理结尾标记
						i++
					}
...
func SEHPEngine(WebNumber int,DataKey string,Path string,Count int,Language string,Source string,Code []string,r *ghttp.Request) string{
	...
	if !PathExists(Config.SEPHWorkFolder+"\\Executable\\"+Path +".exe"){ //执行编译
		//g++ -c sehp.cpp -lws2_32
		//g++ -c test.cpp
		//g++ sehp.o test.o -o test1.exe -lws2_32
		var CodeStr string
		...
		Param1 = append(Param1,Config.SEPHWorkFolder+"\\CompilerTemp\\sehp"+Path+".o")
